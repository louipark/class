...
public class ReviewsDAO {
  ...
  public boolean addReview(ReviewsBean bean) throws DBException{
		...
		try {
			...
			ps = conn
					.prepareStatement("INSERT INTO reviews(mid, pid, reviewdate, descriptivereview, rating, title)  VALUES (?,?,(CURRENT_TIMESTAMP),?,?,?)");
   ...
   * @throws DBException 
   */
  /* below method is totally not used in project
		...
		try {
			...
			if (!isRateable) {
				try {
					conn = factory.getConnection();
					ps = conn
							.prepareStatement("SELECT * FROM officevisits WHERE PatientID=? AND HCPID=?");
					ps.setLong(1, mid);
					ps.setLong(2, pid);
					rs = ps.executeQuery();
					isRateable = rs.next();
